...
class Directory:
    ...
    def get_wavs(self):
        ...
        wavs = [f for f in paths if re.search(self.regexp_include, f)]
        if self.regexp_exclude:
            wavs = [f for f in paths if not re.search(self.regexp_exclude, f)]
        if len(wavs) == 0:
            return []
        processed = [f[:-5] for f in files if f[-5:] == ".json"]
        output = [f for f in wavs if f[:-4] not in processed]
        return output
